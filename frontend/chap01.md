...
本课程的目标是让你掌握 Flutter、Java、MySQL、Docker 等技术栈，从而让你成为一个全栈开发工程师。本课程的技术内容包括：

- Flutter 路由，BLoC 模式，Repository 模式，Dio 网络配置，拦截器，缓存，文件上传，文件下载，图片加载，状态管理，表单组件，表单验证等等，
- 基于 Spring Boot 3.x 的 RESTful API 开发，Spring Data JPA 数据访问层，命名规则查询，Example查询，Specification 高级查询，动态查询，数据库实体类数据类型转换器，JSON 序列化/反序列化转换器，CORS 跨域资源共享，全局异常处理，RFC 7807 问题详情，全局参数处理，Swagger 文档
- 容器化部署，多容器编排，nginx 反向代理配置，容器环境变量，Graylog 统一日志分析
...
- 第一章：课程介绍。介绍课程的目标，内容，结构，学习方法以及需求的概要分析。
- 第二章：环境搭建和项目介绍。介绍 Flutter 和 Java 的环境搭建，以及项目的结构和技术选型。
  - Flutter 环境搭建
  - 项目结构
  - Java 环境搭建
  - Gradle 介绍
  - Lombok 介绍
  - IDEA 插件安装
- 第三章：完成 App 原型的图片区块。我们通过一个图片区块的需求来学习如何从前端角度进行需求分析，如何进行原型设计，如何进行验证，以及如何进行改进。涉及到的知识点有：
  - 适配不同的屏幕尺寸的布局
  - 利用扩展方法简化代码
  - 利用函数式方法简化代码
  - 模型的抽象和验证
- 第四章：完成 App 原型的商品和瀑布流区块：本章完成剩下的区块，仍然是从前端角度进行需求分析，原型设计，验证和改进。涉及到的知识点有：
  - 富文本的处理
  - 领域模型的泛型设计
  - 上拉加载更多
  - 下拉刷新
  - 异步加载假数据
- 第五章：用 Spring Boot 3.x 构建动态图片和文件管理 API。
  - HTTP 协议
  - RESTful API 设计
  - Spring Boot 3.x 的 RESTful API 开发
  - 参数校验
  - Swagger 文档
  - 全局异常处理
  - RFC 7807 问题详情
